---
name: execute-phase
description: Execute the current phase — the main engineering analysis loop
---
...
Read `{GED_INSTALL_DIR}/specs/workflows/execute-phase-workflow.md` first. Do NOT improvise.
...
### Pre-execution
1. Load state via ged-state
2. Check conventions via ged-conventions
...
2. For each plan/task in the wave (parallel):
   a. Spawn ged-executor subagent with task context
...
4. Run inter-wave verification if configured (dimensional/equilibrium checks)
5. Update STATE.md

### Post-Phase Verification
After all waves complete:
1. Spawn ged-verifier with all phase artifacts
