"""MCP server for GED project state management.
...
from ged.core.constants import get_layout
from ged.core.state import StateEngine, ProjectState

server = Server("ged-state")
...
async def list_tools():
    return [
        Tool(
            name="ged_state_load",
            description="Load the current GED project state (phases, conventions, decisions, metrics).",
            inputSchema={
                "type": "object",
                "properties": {
                    "project_dir": {
...
def _engine(project_dir: str | None = None) -> StateEngine:
    if project_dir:
        from ged.core.constants import ProjectLayout
...
async def call_tool(name: str, arguments: dict):
    try:
        ...
        elif name == "ged_state_advance_phase":
            engine = _engine(arguments.get("project_dir"))
            engine.advance_phase(arguments["phase_id"])
            state = engine.load()
            return [TextContent(type="text", text=json.dumps({"status": "ok", "current_phase": state.current_phase}))]

        elif name == "ged_state_set_result":
            engine = _engine(arguments.get("project_dir"))
